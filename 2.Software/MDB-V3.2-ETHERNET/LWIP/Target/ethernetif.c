...
/* USER CODE BEGIN LOW_LEVEL_INIT */
  ...
  /* LAN9303 特殊配置:
   * - STM32 连接到 PHY0 (虚拟 CPU 端口)
   * - PHY0 显示 Link=UP, 但默认是 10M Half-Duplex
   * - 需要强制配置为 100M Full-Duplex 并启动 DMA
   */
  if (hal_eth_init_status == HAL_OK)
  {
    ...
    // 读取 PHY0 当前配置
    HAL_ETH_ReadPHYRegister(&heth, 0, 0, &regvalue);  // BMCR
    printf("[ETH] PHY0 当前 BMCR=0x%04lX\r\n", regvalue);
    
    // 强制 PHY0 配置为 100M Full-Duplex
    regvalue = 0x2100;  // Bit 13=Speed(100M), Bit 8=Full-Duplex
    HAL_ETH_WritePHYRegister(&heth, 0, 0, regvalue);
    HAL_Delay(50);
    
    // 配置 MAC 为 100M Full-Duplex
    ...
    else
    {
      ...
      // 验证 DMA 状态
      printf("[ETH] DMA SR = 0x%08lX, DSR = 0x%08lX\r\n", 
             heth.Instance->DMACSR, heth.Instance->DMADSR);
    }
  }
